    ...
    <div className="min-h-screen bg-mesh text-white flex flex-col">
      ...
      <div className="pt-safe px-4 pb-3">
        <div className="flex items-center gap-3 mt-3 mb-3">
          ...
          <span className="text-green-400/40 text-sm">{hikes.length}회</span>
        </div>
        ...
        {hikes.length > 0 && (
          <div className="glass rounded-2xl p-3 grid grid-cols-2 gap-2 mb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-green-400/40 text-xs">총 거리</p>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-green-400/40 text-xs">총 시간</p>
        ...
        <div className="glass-hi rounded-2xl flex items-center gap-2 px-3 py-2.5">
          <Search className="w-4 h-4 text-green-400/50 flex-shrink-0" />
          <input
            value={query}
      ...
      <div className="flex-1 overflow-y-auto pb-safe px-4">
        {loading ? (
          <div className="flex items-center justify-center h-40 text-green-400/40 text-sm">불러오는 중...</div>
        ) : filtered.length === 0 ? (
  ...
  return (
    ...
    >
      ...
      <div className="w-12 h-12 rounded-2xl flex-shrink-0 flex items-center justify-center"
        style={{ background: 'rgba(34,197,94,0.15)', border: '1px solid rgba(74,222,128,0.2)' }}>
      ...
      <div className="flex-1 min-w-0">
        <p className="text-white font-medium text-sm truncate mb-0.5">
          {route || <span className="text-green-400/30">루트 미입력</span>}
        </p>
        <p className="text-green-400/40 text-xs mb-1.5">{formatDate(hike.date)}</p>
        <div className="flex items-center gap-2">
          <span className="text-green-300/70 text-xs font-medium">{hike.distance.toFixed(1)} km</span>
          <span className="w-1 h-1 rounded-full bg-green-400/20" />
          <span className="text-green-300/70 text-xs">{formatDuration(hike.duration)}</span>
          <span className="w-1 h-1 rounded-full bg-green-400/20" />
          <span className="text-green-300/70 text-xs flex items-center gap-0.5">
            <TrendingUp className="w-3 h-3" />{hike.maxElevation}m
          </span>
        </div>
      </div>

      <ChevronRight className="w-4 h-4 text-green-400/30 flex-shrink-0" />
    </button>
  )
}
